...
class GetWattageVariationsUseCase : BaseUseCase<List<FilterVariationCF>>() {
    ...
    override suspend fun useCaseExecution(params: Array<out Any?>): DataState<List<FilterVariationCF>> {
        ...
        productList.forEach {
            if (it.colorCctCode == productSelected?.colorCctCode && it.productFinishCode == productSelected.productFinishCode) {
                it.isAvailable = true
                filterHashSet.add(
                    FilterVariationCF(
